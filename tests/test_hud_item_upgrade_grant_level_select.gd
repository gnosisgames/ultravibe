...
## ItemUpgradeGrantBlueLevelUp on level select before PlayLevel — full HUD juice path.
...
func _initialize() -> void:
	print("--- HUD Item Upgrade Grant Level Select Test ---")
		...
		1:
			if _frames < 180:
				if _m3 != null and not _m3.is_consumable_use_presentation_active():
					if _verify():
						print("[SUCCESS] Item upgrade grant consumable on level select")
						print("--- HUD Item Upgrade Grant Level Select Test Passed ---")
						quit(0)
						return true
				return false
			if not _verify():
...
func _setup() -> bool:
	...
	var slot := _consumable_slot_for_id(_m3, "ItemUpgradeGrantBlueLevelUp")
	if slot < 0:
		print("[FAIL] ItemUpgradeGrantBlueLevelUp not in bag")
		return false
...
func _verify() -> bool:
	...
	params.set_key("upgradeId", "BlueLevelUp")
	...
	if payload == null or not payload.is_valid() or not bool(payload.get_node("hasUpgrade").value):
		print("[FAIL] BlueLevelUp not applied after consumable")
		return false
	return true
...
func _fail(reason: String) -> void:
	print("[FAIL] HUD Item Upgrade Grant (%s)" % reason)
	print("--- HUD Item Upgrade Grant Level Select Test FAILED ---")
	quit(1)
