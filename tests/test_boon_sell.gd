...
## Equipped boon can be sold via DeactivateBoon (inventory sell refund).
...
func _initialize() -> void:
	print("--- Boon Sell Test ---")
	var ok := _run()
	print("--- Boon Sell Test %s ---" % ("Passed" if ok else "FAILED"))
	quit(0 if ok else 1)
...
func _run() -> bool:
	...
	if rows.is_empty():
		print("[FAIL] no boon rows")
		return false
	var entry: GnosisNode = rows[0]
	var sell_price := InventoryTooltipUiScript.read_inventory_sell_price(entry)
	if sell_price <= 0:
		print("[FAIL] sellPrice missing on equipped boon")
		return false

	var actions := InventoryTooltipUiScript.build_inventory_row_actions(engine, entry, "boons")
	if actions.is_empty():
		print("[FAIL] sell action not built for boon tooltip")
		return false

	var money_before := _read_money(currency)
	if not InventoryTooltipUiScript.try_sell_boon_entry(match3, entry):
		print("[FAIL] try_sell_boon_entry returned false")
		return false
	if SupportScript.is_boon_catalog_id_equipped(match3, "Glitch"):
		print("[FAIL] Glitch still equipped after sell")
		return false
	...
	if money_after < money_before + sell_price:
		...
		return false
	print("[OK] sold boon for %d (money %d -> %d)" % [sell_price, money_before, money_after])
	return true
