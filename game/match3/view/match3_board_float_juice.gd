...
static func spawn_destroy_gem_popups(
	...
	if _is_disabled_item_type(item_type_id):
		_spawn_popup(parent, anchor, "\u00D7", COLOR_DISABLED, 0.0)
		return
...
static func _spawn_popup(parent: Control, anchor: Vector2, text: String, accent: Color, delay: float) -> void:
	if text.is_empty():
		return
	if delay > 0.0:
		parent.get_tree().create_timer(delay).timeout.connect(
			func() -> void:
				if is_instance_valid(parent):
					_play_popup(parent, anchor, text, accent)
		)
	else:
		_play_popup(parent, anchor, text, accent)


static func _play_popup(parent: Control, anchor: Vector2, text: String, accent: Color) -> void:
	if _font == null:
	...
	var dest_pos := wrap.position + Vector2(0, -RISE_PX)
	...
	tw.chain().tween_property(wrap, "position", dest_pos, maxf(0.15, PRIMARY_LIFETIME - FADE_IN_DURATION)) \
